...
CREATE TABLE `Users` (
  ...
  PRIMARY KEY (`user_id`),
  CONSTRAINT `Users_country_id` FOREIGN KEY (`country_id`) REFERENCES Countries (`country_id`)
)
  ENGINE = InnoDB
...
CREATE TABLE `User_Role` (
  ...
  `role_id` INT(10) UNSIGNED NOT NULL,
   CONSTRAINT `User_Role_role_id`  FOREIGN KEY (`role_id`) REFERENCES Roles (`role_id`),
   CONSTRAINT `User_Role_user_id`  FOREIGN KEY (`user_id`) REFERENCES Users (`user_id`)
)
  ENGINE = InnoDB
  DEFAULT CHARSET = utf8;
...
CREATE TABLE `Role_Permission` (
  ...
  CONSTRAINT `Role_Permission_role_id` FOREIGN KEY (`role_id`) REFERENCES Roles (`role_id`),
  CONSTRAINT `Role_Permission_perm_id` FOREIGN KEY (`perm_id`) REFERENCES Permissions (`perm_id`)
)
  ENGINE = InnoDB
  DEFAULT CHARSET = utf8;
